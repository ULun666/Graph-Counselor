...
# 啟動 Qwen2.5-7B-Instruct 模型，使用單張 GPU
...
# 註解掉 Qwen2.5-72B-Instruct 模型的啟動指令
...
# 將所有模型版本都指向 7B 模型
GPT_version=./model/Qwen2.5-7B-Instruct 
EVAL_GPT_version=Qwen2.5-7B-Instruct
SIMPLE_MODEL_NAME=Qwen2.5-7B-Instruct
REFLECT_version=Qwen2.5-7B-Instruct
...
do
    if [ "$DATASET" != "maple" ]; then
        ...
        python ./code/run.py --dataset $DATASET \
                    ...
                    --api_url3 http://0.0.0.0:8010/v1/completions > ./scripts/Qwen2.5-7B-Instruct-$DATASET-plan_reflect_llm7b_multiple.log
        curl -s http://0.0.0.0:8010/v1/completions > /dev/null
        if [ $? -ne 0 ]; then
            echo "vllm fails, please check the log"
            exit 1
        ...
        do
            ...
            python ./code/run.py --dataset $DATASET \
                    ...
                    --judge_correct $judge_correct\
                    --reflect_prompt $prompt\
                    --api_url3 http://0.0.0.0:8010/v1/completions > ./scripts/Qwen2.5-7B-Instruct-$DATASET-$maple-plan_reflect_llm7b_multiple.log
        done
    fi
done
